...
public class AutorService {
    ...
    public Autor findById(Long id) { return autorRepo.findById(id).orElseThrow(); }
    public Autor update(Long id, Autor a) { Autor ex = autorRepo.findById(id).orElseThrow(); ex.setNombre(a.getNombre()); ex.setApellido(a.getApellido()); ex.setUpdatedAt(Instant.now()); return autorRepo.save(ex); }
    public void delete(Long id) { autorRepo.deleteById(id); }
}
